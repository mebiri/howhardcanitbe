...
if opts.using_eos_for_prior and opts.using_eos:
    ...
    print("Hold up! Something's wrong with the EOS arguments!")
    sys.exit(0)
...
if opts.supplementary_likelihood_factor_code and opts.supplementary_likelihood_factor_function:
  ...
  if opts.using_eos_for_prior:
          ...
          if hasattr(external_likelihood_module,'retrieve_eos'):
              print("--Retrieving EOS from external module--")
...
if opts.aligned_prior == 'alignedspin-zprior':
    print("Modifying priors according to alignedspin-zprior")
...
#    from sklearn.ensemble import RandomForestRegressor
    ...
    if y_errors is None:
        rf.fit(x,y)
        print("y_errors is None")
    else:
        print("y_errors in not None; applying fit with weights")
    ...
    def fn_return(x_in,rf=rf):
        ...
        print("indx_ok, initially:",indx_ok)
        ...
        print("indx_ok size:",indx_ok_size)
        indx_ok = np.logical_and(indx_ok, indx_ok_size)
        print("indx_ok, finally:",indx_ok)
...
if opts.input_tides:
    print(" Tides input")
    col_lnL +=2 #col_lnL = 11
    col_lambda1 = col_lnL -2 #col_lambda1 = 9 (tides come before lnL in all.net)
    if opts.input_eos_index: #don't use this, currently
...
elif opts.use_eccentricity: #don't use this, currently
...
print("Analyzing dat; first line printouts:")
tag = 1
for line in dat:
  if tag == 1: print("Here is the line:\n",line)
  ...
  if tag == 1: print("mc_here:",mc_here)
  ...
  if line[col_lnL] < opts.lnL_peak_insane_cut:
    ...
    if tag == 1: print("line_out length:",len(line_out))
 ...
 #        line_out[x] = getattr(P, coord_names[x])
    ...
    if tag == 1: print("length of dat_out:",len(dat_out))
    ...
    # Mirror!
    if opts.mirror_points: #not using (?)
        if tag == 1: print("Mirroring points.")
        P.swap_components()
        # INPUT GRID: Evaluate binary parameters on fitting coordinates
        line_out = np.zeros(len(coord_names)+2)
        for x in np.arange(len(coord_names)):
        ...
        for x in np.arange(len(line_out)):
            ...
            if low_level_coord_names[x] in ['mc']:
                mc_index = x
        dat_out_low_level_coord_names.append(line_out)
  if tag == 1: print("---End of first loop iteration.---")  
  tag = 2
...
#use_portfolio=False
if opts.sampler_method == "AV": #USING THIS
    sampler = mcsamplerAdaptiveVolume.MCSampler()
    opts.internal_use_lnL= True  # required! - weird, arg says only viable for GMM...
...
##
print(" Preparing sampling low_level_coords:", low_level_coord_names)
for p in low_level_coord_names:
    ...
    if p == 'mc' and opts.mc_range:
        ...
        print("mc range eval'd to:",range_here)
    elif p=='mc' and opts.trust_sample_parameter_box:#not using
        range_here = [np.max([range_here[0],mc_min]), np.min([range_here[1],mc_max])]
    if p =='mtot' and opts.mtot_range:#not using
    ...
    if (p=='ln_mu1' or p == 'mu1' or p=='mu2') and opts.trust_sample_parameter_box:#not using
    ...
    adapt_me = True
    if p in opts.no_adapt_parameter:#not using
        adapt_me=False
...
print("Sampler parameters added.")
...
if opts.sampler_method == 'NFlow':#not using this
    extra_args['n_adapt'] = 10  # reduce this?
tempering_adapt=True
if opts.force_no_adapt:   #probably not used
...
if hasattr(sampler, 'setup'):#not using?
    extra_args_here = {}
...
# =============================================================================
# if sampler_oracle:  # NON-PORTFOLIO SCENARIO TARGET #not using
...
###############################################################################
#NONE OF THE BELOW IS BEING REACHED--------------------------------------------
###############################################################################
