...
parser.add_argument("--single",default=True,help="For testing; will only make composite file for first line in synthetic data file.") #TODO: make False by default
parser.add_argument("--plot-composite",default=False,help="Scatterplot of generated points, using lnL as color scale.") 
...
def generate_obs_grid(m1, m2, rv, idx):
    ...
    if opts.plot_composite:
        plot_composite(data_grid)
...
def plot_composite(grid):
    import matplotlib.pyplot as plt
    
    #Scatterplot:
    fig1 = plt.figure(figsize=(8,5),dpi=250) 
    ax = fig1.add_subplot(111)
    ax.scatter(grid[:,1],grid[:,2],c=grid[:,9])
